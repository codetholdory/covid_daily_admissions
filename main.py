...
def covid19_tweet(event, context):
    if check_last_modified():
        ...
        data, latest_7day_average, latest_update_date, latest_admissions = add_7_day_average(raw_data)
        create_graph(data, latest_7day_average)
        create_tweet(latest_7day_average, latest_update_date, latest_admissions)
        create_toot(latest_7day_average, latest_update_date, latest_admissions)
    else:
...
def get_local_last_modified():
    try:
        ...
        local_last_modified = download_blob(storage_bucket, "local_admissions_last_modified")
    except:
        print("Could not access", str(storage_bucket), "local_admissions_last_modified")
...
def write_last_modified_to_file(last_modified):
    ...
    upload_blob(storage_bucket, "local_admissions_last_modified", last_modified_string)
...
def add_7_day_average(data):
    ...
    latest_admissions = latest_data["newAdmissions"]
    return data, str(latest_7day_average), str(latest_update_date), str(latest_admissions)
...
def create_graph(data, latest_7day_average):
    ...
    plt.title("COVID-19 7-Day Average Admissions England - " + latest_7day_average)
    plt.savefig(graph_file)


def create_tweet(latest_7day_average, latest_update_date, latest_admissions):
...
def create_toot(latest_7day_average, latest_update_date, latest_admissions):
